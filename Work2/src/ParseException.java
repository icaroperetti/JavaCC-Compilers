  ...
  private static String initialise(Token currentToken,
    ...
    for (int i = 0; i < expectedTokenSequences.length; i++) {
      ...
      for (int j = 0; j < expectedTokenSequences[i].length; j++) {
    	if (tokenImage[expectedTokenSequences[i][j]].equals("<NUMBER>")) {
    		expected.append("\"0 - 9\"").append(' ');
    	} else if (tokenImage[expectedTokenSequences[i][j]].equals("<EOF>")) {
    		expected.append("Fim da expressão").append(' ');
    	} else {
    		expected.append(tokenImage[expectedTokenSequences[i][j]]);
    	}
    	if (expectedTokenSequences[i].length == j - 1) {
